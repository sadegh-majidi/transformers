...
from .utils import is_flash_attn_1_available, logging
...
if is_flash_attn_1_available():
    ...
    from flash_attn.flash_attn_interface import flash_attn_unpadded_func, flash_attn_func
...
def repeat_kv_3d(hidden_states: torch.Tensor, n_rep: int) -> torch.Tensor:
    batch, n_kv_heads, head_dim = hidden_states.shape
    if n_rep == 1:
        return hidden_states
    hidden_states = hidden_states[:, :, None, :].expand(batch, n_kv_heads, n_rep, head_dim)
    return hidden_states.reshape(batch, n_kv_heads * n_rep, head_dim)


def repeat_kv_4d(hidden_states: torch.Tensor, n_rep: int) -> torch.Tensor:
...
def _flash_attention_forward(
    ...
    **kwargs,
    ...
    flash_kwargs["deterministic"] = deterministic if deterministic is not None else False
    ...
    if attention_mask is not None:
        ...
        key_states = repeat_kv_4d(key_states, kwargs.get("num_groups", 1))
        value_states = repeat_kv_4d(value_states, kwargs.get("num_groups", 1))
    ...
    # Use `flash_attn_varlen_func` to prevent cross-example attention and also allow padding free approach
        ...
        raise RuntimeError("FA1: No shot")
